...
session after `DATABASE_URL` became available.
...
2. `npm run db:seed` creates the demo clinic, demo user, and 8 leads.
...
npm run db:seed -> Seeded clinic "Bright Smile Dental"
                   ...
                   Demo staff login: owner@bright-smile-demo.test / [REDACTED-REVOKED-DEMO-PASSWORD]
...
membership. It deletes and recreates only the demo clinic and demo user, so it is repeatable. The
demo password is hashed through Better Auth so it is compatible with real sign-in.
